        ...
        if let Ok(content) = fs::read_to_string("config.toml") {
            ...
            for line in content.lines() {
                ...
                if let Some(pos) = line.find('=') {
                    ...
                    if ["reports_dir", "tcode", "variant", "layout", "column_name", 
                        "date_range_start", "date_range_end"].contains(&key) {
                        continue;
                    }
...
pub fn handle_configure_sap_params() -> Result<()> {
    ...
    loop {
        ...
        match selection {
            ...
            4 => {
                // Configure Date Range
                let current_start = config.date_range.as_ref().map(|(s, _)| s.clone()).unwrap_or_default();
                let current_end = config.date_range.as_ref().map(|(_, e)| e.clone()).unwrap_or_default();
            ...
            5 => {
                ...
                if param_value.is_empty() {
                    ...
                    config.additional_params.insert(param_name.clone(), param_value.clone());
